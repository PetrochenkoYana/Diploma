...
namespace CSML
{
    public class Matrix
    {
        ...
        public Matrix(string matrix_string)
        {
            ...
            //try
            {
                for (int i = 1; i <= rowCount; i++)
                {
                    ...
                    {
                        this[i, j] = new Complex(double.Parse(curcol[j - 1],fmt));
                    }
                }
            }
        ...
        public Matrix LUSafe()
        {
            ...
            for (int j = 1; j <= n; j++)
            {
                ...
                if (j < n)
                {
                    ...
                    if (this[j, j] == 0)
                        throw new DivideByZeroException("close to 0");
                }
        ...
        public static Matrix operator %(Matrix A, Matrix B)
        {
            var C = new MatrixOfMatrix(A.RowCount, A.ColumnCount);
            ...
            {
                for (int j = 1; j <= C.ColumnCount-1; j++)
                {
                    C[i,1]= Matrix.VerticalConcat(C[i, 1], C[i, j + 1]);
                }
            }
            if (C.RowCount == 1) { resultMatrix = C[1, 1]; }
            else
            {
                resultMatrix = Matrix.HorizontalConcat(C[1, 1], C[2, 1]);
        ...
        public static Matrix operator &(Matrix A, Matrix B)
        { return (A % Matrix.Identity(B.rowCount) ) + (Matrix.Identity(A.rowCount)% B); }
       ...
       public void Print()
            ...
            for (int i = 1; i <= this.RowCount; i++)
            {
                ...
                for (int j = 1; j <= this.ColumnCount; j++)
                {
                    if (j == this.columnCount)
                        Console.Write($"{this[i, j].Re}".Replace(",", ".")+"; ");
                    else
                    Console.Write($"{this[i, j].Re} ".Replace(",", ".")+", ");
                }
        ...
        {
            var res = new double[this.RowCount][];
            for (int i = 0; i < res.Length; i++)
                res[i] = new double[this.ColumnCount];
            for (int i = 1; i <=this.RowCount; i++)
            {
                for (int j = 1; j <=this.ColumnCount; j++)
                {
                    res[i - 1][j - 1] = this[i, j].Re;
                }
            }
            return res;
        }
        public static  Matrix CastingToMatrix(double[][] p)
        {
            var res = new Matrix(p.Length, p[0].Length);
            for (int i = 1; i <= p.Length; i++)
            {
                for (int j = 1; j <= p[0].Length; j++)
                {
                    res[i,j].Re = p[i-1][ j-1];
                }
            }
            return res;
        }
